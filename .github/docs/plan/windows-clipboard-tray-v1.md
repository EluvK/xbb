...
- ✅ Step 5 完成：本地“确认后上云”链路已闭环。

### Step 5：本地“确认后上云”链路（已完成）
- 目标：实现“非自动双写”的手动确认同步。
- 已完成项：
  - 历史页支持选中后触发同步（单条/批量）。
  - 同步成功后更新本地状态（`localOnly=false`），失败项保留选择支持重试。
  - 编辑保存按 `localOnly` 分流：
    - `localOnly=true`：仅更新本地；
    - `localOnly=false`：本地更新后立即同步云端（失败落 `failed` 状态）。
  - 删除选中项支持“云端 + 本地”一致删除（已同步数据会删除云端，失败可重试）。
  - 历史页增加下拉刷新，手动触发最新同步。
  - 启动流程接入 Clipboard 自动同步（受“启用剪贴板备份”开关控制）。
- 当前验收：
  - 成功/失败状态可见，成功后本地状态一致更新。
  - 编辑、删除、手动刷新、启动同步场景均可验证本地与云端一致性。
...
- 去重策略在真实复制场景可验证生效。
- 手动确认同步后，本地状态与云端结果一致。
- 编辑已同步记录后可自动回写云端；失败时有可见状态。
- 删除已同步记录时可同步删除云端数据，避免“本地删除但云端残留”。
- 历史页支持下拉刷新触发同步，启动时可自动执行 Clipboard 同步。
