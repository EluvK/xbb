...
class XbbClient {
  ...
  ClientResult<OpenApiGetUserResponse> getUser(
    String? id,
    String? name,
    String auth,
  ) async {
    try {
      ...
      client.badCertificateCallback =
          ((X509Certificate cert, String host, int port) => true);
      String url =
          id != null ? "$baseUrl/user?id=$id" : "$baseUrl/user?name=$name";
...
Future<OpenApiGetUserResponse?> getUser({String? id, String? name}) async {
  final settingController = Get.find<SettingController>();
  var auth = settingController.getCurrentBaseAuth();
  var baseUrl = settingController.serverAddress.value;
  XbbClient client = XbbClient(baseUrl: baseUrl);
  return await client.getUser(id, name, auth).getOrNull();
}
